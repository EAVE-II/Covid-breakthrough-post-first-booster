...
df_cohort_vacc_g$bmi_gp <- relevel(df_cohort_vacc_g$bmi_gp, ref = "20-24")
######### Modelling
# crude analysis
# sex
...
glm_pos1 <- glm(event~offset(log(p_years))  +date_period+ Sex + age_gp + vacc_gap+ variant+
                  ...
                  ur_combined+bmi_gp + HB , family=poisson,weights = weight,data=df_cohort_vacc_g)
...
for (i in 1:length(conditions)){#length(conditions)
  ...
  glm_pos <- glm(event~offset(log(p_years))  +get(conditions[i])+ Sex + age_gp + variant +
                   simd2020_sc_quintile + risk_gp + variant*get(conditions[i]), 
                 family=poisson,weights = weight,data=temp)
  #summary(glm_pos)
  z<-as.data.frame(round(exp(glm_pos$coefficients),3))
  z<-cbind(z,round(exp(glm_pos$coefficients-coef(summary(glm_pos))[,2]*1.96),2))
