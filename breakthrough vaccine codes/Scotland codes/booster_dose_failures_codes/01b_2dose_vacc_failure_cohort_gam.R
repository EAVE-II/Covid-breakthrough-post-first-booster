...
df_cohort <- readRDS(paste0(project_path,"/data/df_cohort_15-03-2022.rds"))
covid_hosp_death <- readRDS(paste0(project_path,"/data/covid_hosp_death_15-03-2022.rds"))
covid_death <- readRDS(paste0(project_path,"/data/covid_death_15-03-2022.rds"))
covid_hospitalisations <- readRDS(paste0(project_path,"/data/covid_hospitalisations_15-03-2022.rds"))
Vaccinations <- readRDS(paste0(project_path,"/data/Vaccinations_15-03-2022.rds"))
project_path <- paste0(Location,"EAVE/GPanalysis/progs/UA/booster_dose_failures")
covid_hosp_death <- filter(covid_hosp_death,admission_date<="2022-01-15")
covid_hosp_death <- filter(covid_hosp_death,SpecimenDate<="2022-01-15")
...
z_df <- Vaccinations %>% filter(!is.na(date_vacc_3)) %>% 
  filter(flag_incon==0) %>% 
...
z <- z %>% filter(is.na(event_date) | event_date > date_vacc_3) #omit those with event before vaccination
z <- z %>% mutate(event = if_else(is.na(event_date), 0L,1L)) %>% 
  mutate(event_date = if_else(is.na(event_date), a_end ,event_date))
z <- z %>% filter(date_vacc_3 < event_date) #drop any vaccinated after end of study
print(sum(z$event))

a_begin <- min(z$date_vacc_3)
...
df_all <- filter(df_all, !is.na(date_vacc_2))
...
df_all <- df_all %>%  mutate(days_between_vacc = as.numeric(date_vacc_3-date_vacc_2)) %>% 
  mutate(vacc_gap_b = cut(days_between_vacc, breaks=c(1, 180, max(days_between_vacc)),
                        labels=c("<=6 months","6+ months")))
#get positive test before vaccination variable
z <- df_all %>% 
  dplyr::select(EAVE_LINKNO, date_vacc_3)
z <- z %>% 
  left_join(Positive_Tests, by="EAVE_LINKNO") %>% 
  filter(!is.na(date_ecoss_specimen)) %>% 
  filter(date_ecoss_specimen < date_vacc_3) #keep thise sample before vaccination
z <- z %>% 
...
z <- z %>% 
  ...
  filter(date_ecoss_specimen < date_vacc_3) #keep those sample before vaccination
...
df <- df %>% mutate(days_covpos_dose3 = as.Date(date_vacc_3)+13 - as.Date(SpecimenDate_pos))
df$days_covpos_dose3 <- as.numeric(df$days_covpos_dose3)
df <- df %>% mutate(months_covpos = (lubridate::interval(SpecimenDate_pos, (date_vacc_3+13))) %/% months(1))
df <- df %>% mutate(months_covpos_grp = if_else(days_covpos_dose3 < 0,10000,days_covpos_dose3))

# Create variable for number of months between prior infection and dose 2, then convert to factor:
df <- df %>% mutate(months_covpos_grp = case_when(days_covpos_dose3 > 0 & months_covpos %in% 0:2.9 ~ "0-3 month",
...
df <- df %>% mutate(prior_pos_timing = case_when(SpecimenDate_pos <= (date_vacc_1+13) ~ "Pre d1",
                                                 ...
                                                 SpecimenDate_pos > (date_vacc_2+13) & SpecimenDate_pos < (date_vacc_3+13) ~ "Between d3 & d2",
                                                 SpecimenDate_pos == (date_vacc_3+13) ~ "Between d3 & d2",
                                                 months_covpos_grp == "No prior infection" ~ "No prior infection"))
addmargins(table(df$prior_pos_timing,df$event))

# Change names of new variables for clarity:
names(df)
names(df)[names(df) == 'days_covpos_dose3'] <- 'prior_infect_days'
...
df_all<-df
df_all <- filter(df_all,is.na(covid_hosp_death_date)|covid_hosp_death_date<="2022-01-15")
###########################
seed <- 21021954
...
z_df <- data.frame(EAVE_LINKNO=c(event_ids, no_event_ids)) %>% 
  left_join(dplyr::select(df_all, EAVE_LINKNO, date_vacc_3), by="EAVE_LINKNO") %>% 
  left_join(z_event, by="EAVE_LINKNO") %>% 
  mutate(event = if_else(is.na(event_date), 0L,1L)) %>% 
  mutate(event_date = if_else(is.na(event_date), a_end,event_date))
z_df<-distinct(z_df,EAVE_LINKNO,date_vacc_3,SpecimenDate,event_date,event)
print(sum(z_df$event))

df<- left_join(z_df,df_all,by=
                c("EAVE_LINKNO","date_vacc_3","SpecimenDate","event_date","event"))
df <- distinct(df,EAVE_LINKNO,date_vacc_3,SpecimenDate,event_date,event,.keep_all = TRUE)
...
######
df_cohort_vacc <- filter(df, !is.na(date_vacc_3)) %>%
...
df_cohort_vacc<-mutate(df_cohort_vacc,analysis_end_date_c=if_else(analysis_end_date<date_vacc_3,a_end, analysis_end_date))
df_cohort_vacc<-filter(df_cohort_vacc,date_vacc_1<=a_end)
df_cohort_vacc<-filter(df_cohort_vacc,date_vacc_2<=a_end)
...
#                        date_preiod_0014 = if_else(analysis_end_date_c>(date_vacc_3+13),(date_vacc_3+13),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc,
#                        date_preiod_1421 = if_else(analysis_end_date_c>(date_vacc_3+20),(date_vacc_3+20),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc,
#                        date_preiod_2128 = if_else(analysis_end_date_c>(date_vacc_3+27),(date_vacc_3+27),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc,
#                        date_preiod_2835 = if_else(analysis_end_date_c>(date_vacc_3+34),(date_vacc_3+34),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc,
#                        date_preiod_3542 = if_else(analysis_end_date_c>(date_vacc_3+41),(date_vacc_3+41),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc,
#                        date_preiod_4249 = if_else(analysis_end_date_c>(date_vacc_3+48),(date_vacc_3+48),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc,
#                        date_preiod_4956 = if_else(analysis_end_date_c>(date_vacc_3+55),(date_vacc_3+55),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc,
#                        date_preiod_5663 = if_else(analysis_end_date_c>(date_vacc_3+62),(date_vacc_3+62),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc,
#                        date_preiod_6370 = if_else(analysis_end_date_c>(date_vacc_3+69),(date_vacc_3+69),analysis_end_date_c))
# df_cohort_vacc<-mutate(df_cohort_vacc, date_preiod_70o = analysis_end_date_c)

df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_0014 = if_else(analysis_end_date_c>(date_vacc_3+13),(date_vacc_3+13),analysis_end_date_c))
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_1428 = if_else(analysis_end_date_c>(date_vacc_3+27),(date_vacc_3+27),analysis_end_date_c))
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_2842 = if_else(analysis_end_date_c>(date_vacc_3+41),(date_vacc_3+41),analysis_end_date_c))
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_4256 = if_else(analysis_end_date_c>(date_vacc_3+55),(date_vacc_3+55),analysis_end_date_c))
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_5670 = if_else(analysis_end_date_c>(date_vacc_3+69),(date_vacc_3+69),analysis_end_date_c))
df_cohort_vacc<-mutate(df_cohort_vacc, date_preiod_70o = analysis_end_date_c)

########

df_cohort_vacc_g<-gather(df_cohort_vacc,"date_period","end_date",date_preiod_0014,
                         date_preiod_1428,date_preiod_2842,date_preiod_4256,
                         date_preiod_5670,date_preiod_70o)
...
df_cohort_vacc_g$start_date<-coalesce(df_cohort_vacc_g$start_date,df_cohort_vacc_g$date_vacc_3)
...
saveRDS(df_cohort_vacc_g, paste0(project_path,"/data/df_cohort_vacc_g_13-12-2021_gam.rds"))
